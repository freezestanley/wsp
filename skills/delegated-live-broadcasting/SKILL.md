...
2. **默认优先使用 wake 链，而不是口头承诺轮询**
   ...
   - 如果 runtime 尚未支持 hidden/internal wake,不要回退到 `delivery.mode:"announce"` 直投当前对话; 只允许发送最小泄露 token 文本,例如 `当前进度为：__oc_live__:watch-webgen-demo:42:agent%3Awebgen%3Aproj-demo`。
...
6. 若连续多次 wake 都无新增，且距离上次心跳已超过阈值，可补一条低噪音“当前进度为…”心跳确认
...
- fallback token 若被外层 runtime 回显,也必须保持短且不可读,推荐外层只看到 `当前进度为：__oc_live__...`,不能携带整段调度说明
